...
//
// TriangleEngine: the shared machinery behind the depth-striated beam family
// (Triangle and its automatically-configuring variants). One ranked (h-then-g)
// open list per depth layer, held in a deque indexed from the current frontier;
// successors of a layer-i node go into layer i+1. Subclasses differ only in
// their per-step cascade policy -- how far past the frontier to dive and how to
// adapt that depth -- which they supply by overriding cascade().
//
// Ported from the Fast Downward / Scorpion implementations. The planning-
...
template <class D> struct TriangleEngine : public SearchAlgorithm<D> {
	...
	TriangleEngine(int argc, const char *argv[]) :
...
protected:
	...
	// One step of the search: dive past the frontier per the subclass's policy,
	// expanding eligible nodes. Returns true iff the search should stop (a first
	// solution was found in non-anytime mode).
	virtual bool cascade(D &d) = 0;

	// Hook fired whenever a strictly better incumbent is recorded; adaptive
	// variants reset their epoch counters here.
	virtual void on_incumbent_improved() {
	}

	// Drain stale/closed entries from the top of layer i; return the first
	// expandable node, left on top of the heap (use popcurrent to remove it).
	// NULL if the layer holds nothing eligible.
	Node *peekeligible(int i) {
		while (!open[i].empty()) {
			const OpenEntry &e = open[i].top();
			Node *n = e.node;
	...
	// Pop the entry peekeligible() left on top.
	void popcurrent(int i) {
		open[i].pop();
		if (open[i].empty() && i == maxactive)
			recomputemaxactive();
	}

	// Convenience: peek the first eligible node in layer i and pop it.
	Node *popeligible(int i) {
		Node *n = peekeligible(i);
		if (n)
			popcurrent(i);
		return n;
	}

	// Close `n` and route its successors into layer `succ_layer`. Returns true
	// iff the search should stop (first-solution goal in non-anytime mode).
	bool expandnode(D &d, Node *n, int succ_layer) {
		n->status = CLOSED;
		this->res.expd++;
		...
		for (unsigned int k = 0; k < ops.size(); k++) {
			if (ops[k] == n->pop)
				continue;
			this->res.gend++;
			if (considerkid(d, n, state, ops[k], succ_layer)) {
	...
	// the incumbent once the Edge `e` is out of scope).
